...
def listUser():
      ...
      result = User.query.paginate(page=page, per_page=2)
...
def formEditUser():
      ...
      return render_template('admin_user_update.html',user=user)
...
def listPost():
      ...
      page = request.args.get('page', 1, type=int)
      result = Post.query.paginate(page=page, per_page=2)
      return render_template("admin_post.html",result = result)
#delete post
...
def formAddPost():
      return render_template('admin_post_add.html')
#create new post
@app.route("/addPost", methods=['POST'])
def addPost():
      result = request.form
      if result['title']!='' and result['content']!='':
            post = Post(title=result['title'],content=result['content'])
            db.session.add(post)
            db.session.commit()
            return redirect('listPost')
      else:
...
def formEditPost():
      ...
      return render_template('admin_post_update.html',post=post)
...
def detailPost():
      ...
      return render_template("detail_post.html",result = result)
